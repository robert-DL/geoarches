...
srun --cpu-bind=none --mem-bind=none --gpus-per-node=4 --mem=0 --cpus-per-task=8 python3 main_hydra.py \
    ...
    ++dataloader.dataset.pred_lead_time_hours=${pred_lead_time_hours} \
    ++seed=${seed} \
    ++log=${log} \
    ++entity=${wandb_entity} \
    ...
    ++dataloader.dataset.pred_path=${pred_path} \
    ...
    ++stats.module.residual_stats_path=${residual_stats_path} \
    ...
    ++stats.compute_state_scaler_args.state_normalization="pred" \
    ++module.module.lr=${lr} \
    ++module.module.load_deterministic_model=${pred_module} \
